...
function UserHoldingsTable({ userHoldings, coinsList }) {
  ...
  const filteredUserHoldings = userHoldings.filter(holding => holding.coin_name.toLowerCase().includes(searchedCoin.toLowerCase())); // Enables search functionality by filtering out holdings based on name of coin user is searching using toLowerCase to eliminate case sensitivity
